...
## Create project metadata with uv (from scratch)

If you are starting from an empty folder and want `uv` to generate `pyproject.toml` and `uv.lock`:
...
- `uv init --app --no-package`: creates a script/application-style `pyproject.toml`.
- `uv add mcp`: adds `mcp` to dependencies in `pyproject.toml`.
- `uv lock`: resolves dependencies and writes `uv.lock`.
- `uv sync`: creates `.venv` and installs dependencies from the lock.

Note: if `pyproject.toml` does not already include this, add:
...
This prevents `uv` from trying to build this repo as an installable wheel package.
